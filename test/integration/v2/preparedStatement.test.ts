...
describe("prepared statement", () => {
  ...
  it("check simple query with normalized data", async () => {
    const firebolt = Firebolt({
      apiEndpoint: process.env.FIREBOLT_API_ENDPOINT as string
    });
  ...
  it("should throw error when not enough parameters provided", async () => {
    const firebolt = Firebolt({
      apiEndpoint: process.env.FIREBOLT_API_ENDPOINT as string
    });
  ...
  it("should throw error when incorrect parameters provided", async () => {
    const firebolt = Firebolt({
      apiEndpoint: process.env.FIREBOLT_API_ENDPOINT as string
    });
    ...
    expect(
      ...
      'Parameter "$2" is missing from the named parameters.'
    );
  });
});
