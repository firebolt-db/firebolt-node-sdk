...
const mockRequest = jest.fn().mockImplementation(() => {
    return {
        ready: jest.fn().mockImplementation(() => Promise.resolve({
            "engine_id": 11,
            "engine": { id: "", name: "", description: "", endpoint: "", current_status_summary: "" },
...
describe("connection user agent", () => {
    it("propagation", async () => {
        ...
        expect(mockRequest.mock.calls[2][2]["headers"]["user-agent"]).toContain("NodeSDK");
    });
    it("customisation", async () => {
        ...
        await statement.fetchResult();
        expect(mockRequest.mock.calls[2][2]["headers"]["user-agent"]).toContain("NodeSDK");
        expect(mockRequest.mock.calls[2][2]["headers"]["user-agent"]).toContain("ClientA/1.1.1");
        expect(mockRequest.mock.calls[2][2]["headers"]["user-agent"]).toContain("DriverA/2.2.2");
    });
});