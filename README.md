...
The Promise API **is not recommended** for `SELECT` queries with large result sets. This is because it parses results synchronously, so will block JS thread/event loops and may lead to memory leaks due to peak GC loads.
